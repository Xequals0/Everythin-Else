...
int main(int argc, char *argv[])
{
    ...
    //OpenMP pragmas:
    #pragma omp parallel for private (i, hash) schedule(guided)
	for (j = 0; j < num_hf; j++) {
		for (i = 0; i < wl_size; i++) {
    ...
    //OpenMP pragmas ~ parallelize the spell check:
    #pragma omp parallel for private (hash) schedule(guided, 200)
	for (j = 0; j < num_hf; j++) {
		hash = hf[j] (word);
		hash %= bv_size;
		if (bv[hash] == 0)
